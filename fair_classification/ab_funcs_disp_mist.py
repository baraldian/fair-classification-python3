...
import cvxpy
from fair_classification import utils as ut

SEED = 1122334455
seed(SEED) # set the random seed so that the random permutations can be reproduced again
np.random.seed(SEED)

mul_elemwise = cvxpy.multiply
sum_entries = cvxpy.sum
min_elemwise = cvxpy.minimum
...
def train_model_disp_mist(x, y, x_control, loss_function, EPS, cons_params=None):
    ...
    if loss_function == "logreg":
        ...
        loss = sum_entries(cvxpy.logistic( mul_elemwise(-y, x*w) )  ) / num_points # we are converting y to a diagonal matrix for consistent
...
def get_constraint_list_cov(x_train, y_train, x_control_train, sensitive_attrs_to_cov_thresh, cons_type, w):
    ...
    for attr in sensitive_attrs_to_cov_thresh.keys():
        ...
        if index_dict is None: # binary attribute, in this case, the attr_arr_transformed is the same as the attr_arr
            ...
            for ct in [0,1,2]:
                s_val_to_avg[ct][0] = s_val_to_total[ct][1] / (s_val_to_total[ct][0] + s_val_to_total[ct][1]) # N1/N in our formulation, differs from one constraint type to another
...
def get_fpr_fnr_sensitive_features(y_true, y_pred, x_control, sensitive_attrs, verbose = False):
    ...
    for s in sensitive_attrs:
        ...
        for s_val in sorted(list(set(s_attr_vals))):
            ...
            acc = float(cvxpy.sum(y_true_local == y_pred_local)) / len(y_true_local)

            fp = cvxpy.sum(np.logical_and(y_true_local == -1.0, y_pred_local == +1.0)) # something which is -ve but is misclassified as +ve
            fn = cvxpy.sum(np.logical_and(y_true_local == +1.0, y_pred_local == -1.0)) # something which is +ve but is misclassified as -ve
            tp = cvxpy.sum(np.logical_and(y_true_local == +1.0, y_pred_local == +1.0)) # something which is +ve AND is correctly classified as +ve
            tn = cvxpy.sum(np.logical_and(y_true_local == -1.0, y_pred_local == -1.0)) # something which is -ve AND is correctly classified as -ve

            all_neg = cvxpy.sum(y_true_local == -1.0)
            all_pos = cvxpy.sum(y_true_local == +1.0)
...
def get_sensitive_attr_constraint_fpr_fnr_cov(model, x_arr, y_arr_true, y_arr_dist_boundary, x_control_arr, verbose=False):
    ...
    for v in set(x_control_arr):
        ...
        cons_sum_dict[1][v] = cvxpy.sum(np.minimum(0, ((1 - y_arr_true[idx]) / 2) * dist_bound_prod)) * (s_val_to_avg[1][v] / cvxpy.sum(y_arr_true == -1))
        cons_sum_dict[2][v] = cvxpy.sum(np.minimum(0, ((1 + y_arr_true[idx]) / 2) * dist_bound_prod)) * (s_val_to_avg[2][v] / cvxpy.sum(y_arr_true == +1))
